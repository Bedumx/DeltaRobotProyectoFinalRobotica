...
ac0=np.array([.0,.0,.0,.0,.0,.0])
t0=0

q1=np.array([.5,.6,.65,.7,.75,.8])
v1=np.array([.0,.0,.0,.0,.0,.0])
ac1=np.array([.0,.0,.0,.0,.0,.0])
tf=1
...
for joint in UR53:
    ...
    time.sleep(1/100)
for joint in UR54:
    sim.simxSetJointTargetPosition(clientID, joint, (q0[3]), sim.simx_opmode_streaming)
    time.sleep(1/100)
for joint in UR55:
    sim.simxSetJointTargetPosition(clientID, joint, (q0[4]), sim.simx_opmode_streaming)
    time.sleep(1/100)
for joint in UR56:
    sim.simxSetJointTargetPosition(clientID, joint, (q0[5]), sim.simx_opmode_streaming)
    time.sleep(1/100)
...
for t2 in range(s):
    ...
    for joint in UR53:
        ...
        time.sleep(1/100)
    for joint in UR54:
        sim.simxSetJointTargetPosition(clientID, joint, (qd4[t2]), sim.simx_opmode_streaming)
        time.sleep(1/100)
    for joint in UR55:
        sim.simxSetJointTargetPosition(clientID, joint, (qd5[t2]), sim.simx_opmode_streaming)
        time.sleep(1/100)
    for joint in UR56:
        sim.simxSetJointTargetPosition(clientID, joint, (qd6[t2]), sim.simx_opmode_streaming)
        time.sleep(1/100)